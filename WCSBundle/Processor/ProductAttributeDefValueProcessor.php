 ...
 */
class ProductAttributeDefValueProcessor extends AbstractConfigurableStepElement implements ItemProcessorInterface
{
    /** @var Serializer */
    ...
    public function process($product)
    {
        ...
        foreach ($product->getValues() as $value) {
            ...
            if (count($value->getOption()) > 0) {
                $code = $value->getOption();
                if (!empty($code)) {
                    $value->getOption()->setLocale('en_US');
                    $attrCode = $value->getAttribute()->getCode();
                    $attrValue = $value->getOption()->getOptionValue()->getValue();
                }
            }
     ...
     * @return \Aliznet\WCSBundle\Processor\ProductAttributeValueProcessor $language language
     */
